...
export const hariciFilmleriAra = async (sorgu) => {
    ...
    try {
        ...
        return data.results.filter(i => i.media_type !== 'person').map(i => veriyiStandartFormataDonustur(i, 'film'));
    } catch (error) {
...
export const hariciKitaplariAra = async (sorgu) => {
    ...
    try {
        const response = await fetch(url);
        if (!response.ok) throw new Error(`Google Books Hatası: ${response.status}`);
        const data = await response.json();
...
export const icerigiKaydetVeDetaylariCek = async (tmdbId) => {
    ...
    try {
        ...
        const detayliIcerik = {
            ...
            yazar_yonetmen: data.credits.crew.filter(c => c.job === 'Director').map(d => d.name).join(', '),
...
export const kitapKaydetVeDetaylariCek = async (googleBookId) => {
    ...
    try {
        ...
        const detayliKitap = {
            ...
            ozet: info.description ? info.description.substring(0, 1000) : "Açıklama yok.",
